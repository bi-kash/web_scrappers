...
def remove_matching_dicts(list1, list2):
    ...
    filtered_list1 = [d for d in list1 if d['Entity ID Number'] not in entity_id_numbers]
...
def get_records(start_from = None):
    ...
    while True:
        try:    
            current_url = driver.current_url
            rows = driver.find_element(By.ID, "block-sos-content").find_element(By.TAG_NAME, "tbody").find_elements(By.TAG_NAME, "tr")
            ...
            print("There is a error. But don't worry next time you run this script please pass the link to the function from where you want to start")
    ...
    try:
        for data_dict in rows:
            ...
            if len(detailed_dict_list) % 10 == 0:
                ...
                pd.DataFrame.from_records(detailed_dict_list).to_csv("alabama_detailed.csv", index=False)
            random_sleep()
    except:
        print(driver.current_url)
        print(traceback.format_exc())
        print("Do not worry. Make sure load_from_csv is set to True and detailed_csv_exists is set to True")
...
if __name__ == "__main__":
    ...
    if detailed_csv_exists:
        if os.path.exists("alabama_detailed.csv"):
            detailed_dict_list= pd.read_csv("alabama_detailed.csv").to_dict(orient='records')
            data_dict_list = remove_matching_dicts(data_dict_list, detailed_dict_list) # no need to scrape again if data already exists
