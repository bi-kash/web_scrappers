...
def check_overlay(driver: ChromeDriverManager):
    ...
    try:
        ...
        wait.until(
            lambda driver: driver.find_elements(By.CLASS_NAME, "om-effect-overlay")
        )
        action.click()
        action.perform()
    except:
...
def scrapper(
    page_depth: int = 2,
    search_url: str = "https://www.oddsshopper.com/odds/shop/nfl",
    save_to: str = "player_props.csv",
):
    driver = get_driver()
    ...
    for i in range(page_depth):
        ...
        wait.until(
            lambda driver: driver.find_elements(
                By.CLASS_NAME, "MuiGrid-root.MuiGrid-item.MuiGrid-grid-xs-12"
            )
        )

        # projection boxes containing individual informations
        individual_infos = driver.find_elements(
            By.CLASS_NAME, "MuiGrid-root.MuiGrid-item.MuiGrid-grid-xs-12"
        )
        for individual_info in individual_infos:
            ...
            try:
                # Extract date time
                date_time = individual_info.find_element(
                    By.CLASS_NAME,
                    "MuiGrid-root.datetime.MuiGrid-item.MuiGrid-grid-xs-9",
                ).text
                # Extract League name
                league = individual_info.find_element(
                    By.CLASS_NAME, "MuiChip-label.MuiChip-labelSmall"
                ).text
                # Extract player name
                ...
                # Extract stat
                stat = individual_info.find_element(
                    By.CLASS_NAME,
                    "MuiTypography-root.name.MuiTypography-body1.MuiTypography-gutterBottom",
                ).text

                # Extract best bet informations
                best_bet_sport_books = (
                    individual_info.find_element(By.CLASS_NAME, "sportsbook")
                    .find_element(By.TAG_NAME, "h6")
                    .text
                )
                ...
                strengths = individual_info.find_elements(
                    By.CLASS_NAME, "MuiGrid-root.MuiGrid-item.MuiGrid-grid-xs-3"
                )
                projection = ""
                ROI = ""
                win = ""
                ...
                scrapped.append(
                    {
                        ...
                        "Line": best_bet.split(" ")[1],
                        "ROI": ROI,
                        "Win": win,
                        "Hold": hold,
                        "Best Bet": best_bet,
                        "Best Bet Sportsbook": best_bet_sport_books,
                        "Best Bet Odds": best_bet_odds,
                    }
                )
