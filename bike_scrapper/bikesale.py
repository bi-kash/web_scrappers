    ...
    for bike in bikes:
        ...
        for year_temp in years:
            try:
                ...
                year = ""
        price = (
            bike.find_element(By.CLASS_NAME, "price--default")
            .text.split(":")[1]
            .split()[0]
        )
    ...
    for row in rows:
        ...
        labels = [
            ...
            for item in driver.find_element(
                By.CLASS_NAME, "product--properties-col.col--1"
            ).find_elements(By.CLASS_NAME, "product--properties-label")
        ]
        values = [
            item.get_attribute("innerText")
            for item in driver.find_element(
                By.CLASS_NAME, "product--properties-col.col--1"
            ).find_elements(By.CLASS_NAME, "product--properties-value")
        ]
        category = ""
        stock_size = ""
        condition = ""
        ...
        stock_text = driver.find_element(
            By.CLASS_NAME, "delivery--text-available"
        ).get_attribute("innerText")
        row.update(
            {
                "category_shop": category,
                "stock_sizes": stock_size,
                "condition": condition,
