...
# Get lists of link
def get_bike_links(search_url='https://www.cube.eu/de-de/bikes/mountainbike/hardtail?p=1&properties=ad1cb35585ad922fdf2e4a81752bfe9d', driver=None):
...
def scrap_bike(bike_links, driver):
    ...
    for url in bike_links:
        ...
        row = {}
        driver.get(url)
        Img = driver.find_element(By.CLASS_NAME, 'cms-image-container.is-contain').find_element(By.TAG_NAME, 'img').get_attribute('src')
        ...
        Model = ";".join(Model)
        Rrp = driver.find_element(By.CLASS_NAME, 'product-detail-price-container').find_element(By.TAG_NAME, 'meta').get_attribute('content')

        

        try:
            Img2 = driver.find_element(By.CLASS_NAME, 'img-wrapper').find_element(By.TAG_NAME, 'a').get_attribute('href')
        ...
        row.update({'Source Url': url, 'Img':Img, "Model":Model, "Rrp": Rrp, "Img2": Img2, "Model2": Model2, "Rrp2": Rrp2})
        wait.until(lambda driver: driver.find_element(By.CLASS_NAME, 'product-detail-description-accordion'))

        Highlights = driver.find_element(By.CLASS_NAME, 'product-detail-description-accordion').find_elements(By.CLASS_NAME, 'card-header')
        count = 0
        ...
        properties = driver.find_element(By.CLASS_NAME, "product-property-tables").find_elements(By.CLASS_NAME, 'table-row.row.no-gutters')
        for property in properties:
            title = property.find_element(By.CLASS_NAME, 'title').get_attribute('innerText').strip()
            val = property.find_element(By.CLASS_NAME, 'mb-0').get_attribute('innerText').strip()
            row.update( {title:val} )
        
        details = driver.find_element(By.CLASS_NAME, "product-detail-more-information")

        EINSATZKATEGORIE = details.find_element(By.CLASS_NAME, "title").get_attribute('innerText').strip()
        MAXIMALES = details.find_element(By.CLASS_NAME, 'weight-amount').get_attribute('innerText').strip()
        ...
        geometries = driver.find_element(By.CLASS_NAME, 'table.e-geometry-table').find_elements(By.TAG_NAME, 'tr')
        ...
        for geometry in geometries[1:]:
            col_name = geometry.find_element(By.TAG_NAME, 'th').get_attribute('innerText')
            for info in geometry.find_elements(By.TAG_NAME, 'td'):
                ...
                Marketing_text = driver.find_element(By.CLASS_NAME, 'wrapper-text').get_attribute('innerText')
