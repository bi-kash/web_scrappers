...
def scrap_list():
    ...
    while True:
        ...
        for bike in bikes:
            ...
            condition = bike.find_element(By.CLASS_NAME, "product-badge").get_attribute(
                "aria-label"
            )
            brand = model.split()[0]
            prices = bike.find_element(
                By.CLASS_NAME, "product-list__item-price"
            ).find_elements(By.TAG_NAME, "div")
            price = prices[0].text.strip().split()[0].replace(".", "").replace(",", ".")
            rrp = ""
            if len(prices) > 1:
                rrp = (
                    prices[1].text.strip().split()[0].replace(".", "").replace(",", ".")
                )
...
def scrap_each_page(rows):
    ...
    for row in rows:
        ...
        variant_buttons = driver.find_elements(
            By.CLASS_NAME, "colorSwatch__Container-RuNMR"
        )
        ...
        for button in variant_buttons:
            ...
            size_buttons = driver.find_element(
                By.CLASS_NAME, "AttributeSelector__Attributes-hWZvGf"
            ).find_elements(By.CLASS_NAME, "Button__BaseButton-kNsioU")
            dict_size = {}
            for button_ in size_buttons:
                button_.click()
                try:
                    key = (
                        driver.find_element(By.CLASS_NAME, "Toast__Container-eqNNUH")
                        .find_element(By.CLASS_NAME, "TypographyTheme__StyledP-mQqiM")
                        .text
                    )
