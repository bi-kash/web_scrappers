...
wait = WebDriverWait(
    driver, 10
)  # option to wait 10 seconds atleast if some element takes time to load on page


file_dict = {}
for start_date, end_date in date_list:
    if os.path.exists(
        f"{directory}/gvs_{start_date.strftime('%m-%d-%Y')}_{end_date.strftime('%m-%d-%Y')}.csv"
    ):
        continue
    ...
    try:
        ...
        ).click()  # click on filter dropdown
        random_sleep()
        driver.find_element(
            By.XPATH, '//*[@data-value="IncidentDate"]'
        ).click()  # select filter by date from dropdown
        random_sleep()
        date_inputs = wait.until(
            EC.presence_of_all_elements_located((By.CLASS_NAME, "date-picker-single"))
        )  # get date inputs

        # start date calendar actions

        date_inputs[0].click()
        calendar = driver.find_elements(By.CLASS_NAME, "daterangepicker")[
            0
        ].find_element(By.CLASS_NAME, "calendar")
        ...
        for item in calendar.find_elements(
            By.XPATH,
            f'//td[contains(@class, "available") and not(contains(@class, "off")) and text()="{start_date.day}"]',
        ):
            try:
                item.click()
                break
        ...
        for item in calendar.find_elements(
            By.XPATH,
            f'//td[contains(@class, "available") and not(contains(@class, "off")) and text()="{end_date.day}"]',
        ):
            try:
                item.click()
                break
        ...
        new_file_directory = (
            directory
            + f"/gvs_{start_date.strftime('%m-%d-%Y')}_{end_date.strftime('%m-%d-%Y')}.csv"
        )
        ...
        print(
            "Error while scraping. Please checkout screenshot.png in root folder and checkout logs"
        )
